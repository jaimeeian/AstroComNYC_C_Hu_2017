...
#    T_PDF = plt.hist(np.log10(chemT[idx_disc]), bins=140, histtype='step', normed=True, range=[1,8], weights=1./rho[idx_disc])
    ...
    if kk < 10:
        plt.savefig('phase_diagram00{kk}.png'.format(kk=kk))
    elif 10 <= kk < 100:
        plt.savefig('phase_diagram0{kk}.png'.format(kk=kk))

    else:
        plt.savefig('phase_diagram{kk}.png'.format(kk=kk))
...
"""
    ...
    plt.hist2d(np.log10(n_H[idx_in]), np.log10(chemT[idx_in]), range = [(-6,-2), (0,7)],  bins = 100, norm = LogNorm()); plt.xlabel('Density (cm^-3)'); plt.ylabel('Temperature (K)'); plt.title('Phase Diagram')
    
    if kk < 10:
        plt.savefig('if_phase_diagram00{kk}.png'.format(kk=kk))
    elif 10 <= kk < 100:
        plt.savefig('if_phase_diagram0{kk}.png'.format(kk=kk))

    else:
        plt.savefig('if_phase_diagram{kk}.png'.format(kk=kk))


    Cs = sqrt((10/9)*(u))
    Mj = ((math.pi**(5/2))*(Cs**3))/(6*(GRAVCON**(3/2))*(rho**(1/2)))

    jeanscs = ((6*(GRAVCON**(3/2))*(n_H**(1/2))*3200)/(math.pi**(5/2)))**(2/3)
    
    jeansrho = sqrt(((math.pi**(5/2))*(Cs**3))/(6*(GRAVCON**(3/2))*3200))
